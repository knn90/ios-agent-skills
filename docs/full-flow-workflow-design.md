...
- **Test split** — dedicated test dev (dev-1, TDD-first); accepted that its tests may compile only after the integrate merge (dev-1 must name types exactly per the plan).
